...
obj.input("i"); // return ["i love you", "island", "i love leetcode"]. There are four sentences that have prefix "i".
 ...
 code 114, "i love leetcode" should be in front of "ironman". Also we only need to output top 3 hot sentences,
...
class AutocompleteSystem:
    ...
    def __init__(self, sentences: List[str], times: List[int]):
        self.top = TrieNode()
        self.cur_node = self.top
        self.c = ""
        for sen, time in zip(sentences, times):
            self.addSentence(sen, time)

    def addSentence(self, sen, time):
        node = self.top
        for s in sen:
    ...
    def input(self, c: str) -> List[str]:
        ...
        for x in self.c:
            if x not in node.children.keys():
                return res
            node = node.children[x]

        def get_suggestions(node, input):
            if node.hot != 0:
                res.append((-node.hot, input))

            for key in node.children.keys():
                get_suggestions(node.children[key], input+key)

        get_suggestions(node, self.c)
        final = sorted(res)
        final = [f[1] for f in final]
