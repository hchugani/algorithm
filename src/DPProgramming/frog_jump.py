...
class Solution:
    def canCross(self, stones: List[int]) -> bool:
        ...
        def jump(i, k):
            ...
            if a[i]+(k-1) in mapper and mapper[a[i]+(k-1)]>i:
                ret1 = jump(mapper[a[i]+(k-1)], k-1)

            if a[i]+k in mapper and mapper[a[i]+k]>i:
                ret2 = jump(mapper[a[i]+k ], k)

            if a[i]+k+1 in mapper and mapper[a[i]+k+1]>i:
                ret3 = jump(mapper[a[i]+k+1], k+1)
